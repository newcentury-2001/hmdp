...
public class DelayConsumerQueue extends DelayBaseQueue{
    ...
    public synchronized void listenStart(){        if (!runFlag.get()) {            runFlag.set(true);            log.info("[DelayConsumerQueue] 启动监听线程: {}", relTopic);            listenStartThreadPool.execute(() -> {                log.info("[DelayConsumerQueue] 监听线程已启动: {}", relTopic);                while (!Thread.interrupted()) {                    try {                        assert blockingQueue != null;                        log.debug("[DelayConsumerQueue] 等待消息: {}", relTopic);                        String content = blockingQueue.take();                        log.debug("[DelayConsumerQueue] 接收到消息: {}, 内容长度: {}", relTopic, content.length());                        executeTaskThreadPool.execute(() -> {                            try {                                log.debug("[DelayConsumerQueue] 执行消息消费: {}", relTopic);                                consumerTask.execute(content);                                log.debug("[DelayConsumerQueue] 消息消费完成: {}", relTopic);                            }catch (Exception e) {                                log.error("[DelayConsumerQueue] 消费者执行错误: {}", relTopic, e);                            }                        });                    } catch (InterruptedException e) {
                    ...
                    }                }            });        } else {            log.info("[DelayConsumerQueue] 监听线程已经启动: {}", relTopic);        }    }
